...
4.  **Remote/deploy actions require a fresh explicit go-ahead every time**: never run a `--remote` D1 command, `wrangler deploy`, or `git push` to `main` (which triggers the GitHub Actions deploy) without the user saying so for that specific change. Local `--local` D1 work and `wrangler dev` don't need to ask.
